...
function renderAlertCenter() {
    ...
    if (
        currentDataSourceState.selected_source === "company" &&
        currentDataSourceState.rules_status === "provisional_poc"
    ) {
        renderCompanyPocAlerts(badge, summary, alertList);
        return;
    }
...
function renderCompanyPocAlerts(badge, summary, alertList) {
    const critical = currentAlerts.critical_count || 0;
    const warning = currentAlerts.warning_count || 0;
    const alerts = currentAlerts.active_alerts || [];
    const total = alerts.length;
    ...
    alertList.innerHTML = `
        <div class="poc-alert-disclaimer">
            These findings are generated by local PoC fallback rules. They
            are not official company or Grafana alerts.
        </div>
    `;
    ...
    if (alerts.length === 0) {
        alertList.innerHTML += `
            <div class="alert-item info">
                <div>
                    <h3>No provisional findings</h3>
                    <p>No current device evidence matched the local PoC rules.</p>
                </div>
            </div>
        `;
        return;
    ...
    alerts.forEach(alert => {
        ...
        const threshold = Array.isArray(evidence.threshold)
            ? evidence.threshold.join(", ")
            : evidence.threshold;
        const alertKey = alert.alert_id || `${alert.rule_id}:${alert.device_id}`;
        const alertState = alertStates[alertKey] || "active";
        const actionTime = alertStates[`${alertKey}_actionTime`];
        const actionTimeLabel = actionTime ? formatAlertActionTime(actionTime) : "";

        alertList.innerHTML += `
            <div class="alert-item ${escapeHtml(alert.severity)} ${alertState}">
                <div>
                    <div class="poc-alert-heading">
                        <h3>${escapeHtml(alert.device_name || alert.device_id)}</h3>
                        <span class="poc-alert-badge">PoC fallback</span>
                    </div>
                    <p>
                        ${escapeHtml(alert.title)} ·
                        ${escapeHtml(evidence.field || "signal")}:
                        ${escapeHtml(evidence.value)}${escapeHtml(unit)} ·
                        rule ${escapeHtml(evidence.operator || "")}
                        ${escapeHtml(threshold)}
                    </p>
                    <small class="device-subtext">
                        ${escapeHtml(alert.rule_id)} · ${escapeHtml(alert.source?.telemetry || "company telemetry")}
                    </small>
                    <span class="alert-state-pill ${alertState}">
                        ${alertState}
                        ${actionTimeLabel ? `<span class="alert-state-time">${actionTimeLabel}</span>` : ""}
                ...
                <div class="alert-actions">
                    ...
                    ${alertState === "active" ? `
                        <button
                            class="secondary-btn"
                            onclick="acknowledgeAlert('${escapeHtml(alertKey)}')"
                        >
                            Acknowledge
                        </button>
                    ` : ""}
                    ${alertState !== "resolved" ? `
                        <button
                            class="secondary-btn"
                            onclick="resolveAlert('${escapeHtml(alertKey)}')"
                        >
                            Resolve
                        </button>
                    ` : ""}
